...
RUN wget https://registry.npmjs.org/tar/-/tar-7.5.11.tgz -O /tmp/tar.tgz && \
    ...
    wget https://registry.npmjs.org/follow-redirects/-/follow-redirects-1.16.0.tgz -O /tmp/follow.tgz

RUN mkdir -p /tmp/ex-tar /tmp/ex-mini /tmp/ex-qs /tmp/ex-glob /tmp/ex-spawn /tmp/ex-brace /tmp/ex-follow && \
    ...
    tar -xzf /tmp/follow.tgz -C /tmp/ex-follow && \
    ...
    find /opt/shiny-server -name "follow-redirects" -type d -path "*/node_modules/follow-redirects" -exec cp -R /tmp/ex-follow/package/* {}/ \; && \
